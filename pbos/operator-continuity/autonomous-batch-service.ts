    ...
    prepareReadinessQueue(systemId: string, governedRevision: string): import("../production-runtime").MissionQueueItem | undefined {
        ...
        const items: import("../production-runtime").MissionQueueItem[] = [
            { missionId: "playbook-capability-foundation", systemId, title: "Certified Playbook capability foundation",
                dependencies: [], status: "COMPLETE", rationale: `Seven capabilities proven at ${governedRevision}.`,
                approvalRequired: false, evidenceIds: [`repository:${governedRevision}`] },
            ...readinessTasks.map(task => ({ missionId: task.taskId, systemId, title: task.title,
                ...
                status: "QUEUED" as const, rationale: "Awaiting dependency reconciliation.",
                approvalRequired: task.gate !== "AUTOMATED", evidenceIds: [] }))
        ];
