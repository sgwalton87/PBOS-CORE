...
export class GenesisWorkflowService {
    ...
    async prepareDraftBuild(session: GenesisBuildSession, packageLimit = 10, preparedPlan?: GenesisBuildPlan): Promise<PreparedApplicationBuild> {
        ...
        const batchId = this.telemetry?.beginBatch(session.system.systemId, session.sessionId, selectedPackages, {
            repository: session.system.repository, branch, commit: plan.repositoryRevision,
            objective: `Implement ${selectedPackages.length} governed work packages for ${session.system.name}`,
            mission: selectedPackages.map(item => item.title).join("; ")
        });
        ...
        return { plan, branch, pullRequest, workPackageCount: selectedPackages.length, batchId, revision, generatedPaths: materialized.generatedPaths };
    }
